...
import com.cg.inventorydoctorderservice.repository.AppointmentRepository;
...
import com.cg.inventorydoctorderservice.repository.ReviewRepository;
...
public class DoctorService {
	...
	public Doctor update(updateDoctorRequest updatedoctorRequest) {
		...
		departmentRepository.findById(updatedoctorRequest.getDepartmentId())
				.orElseThrow(() -> new HttpClientErrorException(HttpStatus.NOT_FOUND, "Khong tim thay"));
		doctorUpdate.setDepartmentId(updatedoctorRequest.getDepartmentId());
		specialitiesRepository.findById(updatedoctorRequest.getSpecialityId())
				.orElseThrow(() -> new HttpClientErrorException(HttpStatus.NOT_FOUND, "Khong tim thay"));
		doctorUpdate.setSpecialityId(updatedoctorRequest.getSpecialityId());
//		doctorUpdate.setPrice(updatedoctorRequest.getPrice());
...
//    }
	...
	public ListResult<Doctor> filter(String name, String gender, Integer departmentId, Integer specialityId, int size,
			int page, boolean desc, String orderBy) {
	...
	public Doctor updatePrinciple(Principal principal, UpdateDoctorRequestPriciple updateDoctorRequestPriciple)
			throws IOException {
		...
		departmentRepository.findById(updateDoctorRequestPriciple.getDepartmentId())
				.orElseThrow(() -> new HttpClientErrorException(HttpStatus.NOT_FOUND, "Khong tim thay"));
		doctorUpdate.setDepartmentId(updateDoctorRequestPriciple.getDepartmentId());

		specialitiesRepository.findById(updateDoctorRequestPriciple.getSpecialityId())
				.orElseThrow(() -> new HttpClientErrorException(HttpStatus.NOT_FOUND, "Khong tim thay"));
...
////        doctorUpdate.setAvatar(FileUtils.saveAvatar(file, this.location, request));
	...
	public ListResult<Review> filterReview(Principal principal, int size, int page, boolean desc, String orderBy) {
		Doctor doctorUpdate = doctorRepository.findByUsername(principal.getName());
		Specification<Review> documentPatientSpecification = specificationFilter
				.reviewSpecification(doctorUpdate.getId());
		return ListResult
	...
	@Transactional
	public Appointment updateStatusAppointment(UpdateAppointmentRequest updateAppointmentRequest) {
		...
		if (appointment.getStatus() != Status.WAITING) {
			throw new HttpClientErrorException(HttpStatus.NOT_FOUND, "Thông tin cuộc hẹn đã cập nhật");
		}
	...
	public ListResult<Appointment> findAppointmentById(Integer id, int size,
		...
		Specification<Appointment> documentAppointmentSpecification = specificationFilter
				.appointmentOfPatientSpecification(id);
		return ListResult.from(
				appointmentRepository.findAll(documentAppointmentSpecification, pageable(page, size, orderBy, desc)));
	}

	@Transactional
	public Appointment updatePrescriptionAppointment(
			UpdatePrescriptionAppointmentRequest updatePrescriptionAppointmentRequest) {
		Appointment appointment = appointmentRepository.findById(updatePrescriptionAppointmentRequest.getId())
				.orElseThrow(() -> new HttpClientErrorException(HttpStatus.NOT_FOUND, "Khong tim thay"));
		if (appointment.getTrangthai() == false) {
			throw new HttpClientErrorException(HttpStatus.NOT_FOUND, "Thông tin appoitment đã xóa");
		}
//		if (appointment.getStatus() != Status.APPROVED) {
//			throw new HttpClientErrorException(HttpStatus.NOT_FOUND, "Cuộc  hẹn không phải approved");
//		}
		List<String> prescriptionList = new ArrayList<>();
		JsonNode prescriptionNode = updatePrescriptionAppointmentRequest.getPrescription();
		if (prescriptionNode.isArray()) {
			Iterator<JsonNode> iterator = prescriptionNode.elements();
			while (iterator.hasNext()) {
				JsonNode valueNode = iterator.next();
				String value = valueNode.toString();
				prescriptionList.add(value);
			}
		}
		appointment.setListPrescription(prescriptionList);
		appointment.setUpdate_at(new Date());
	...
	public ListResult<Appointment> findAppointmentDictinctById(Principal principal, int size,
			@Min(value = 1, message = "k tim thay trang") int page, boolean desc, String orderBy) {
		Doctor appUser = doctorRepository.findByUsername(principal.getName());
		Specification<Appointment> documentAppointmentSpecification = specificationFilter
				.appointmentOfPatientDictintSpecification(appUser.getId());
		return ListResult.from(
				appointmentRepository.findAll(documentAppointmentSpecification, pageable(page, size, orderBy, desc)));
	}
