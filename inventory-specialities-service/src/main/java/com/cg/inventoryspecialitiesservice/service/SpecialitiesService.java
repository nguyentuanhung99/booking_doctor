...
//    private String location;
    ...
    @Transactional
    public Specialities update(updateSpecialitiesRequest updatespecialitiesRequest,MultipartFile file) throws IOException {
        Specialities specialitiesUpdate = specialitiesRepository.findById(updatespecialitiesRequest.getId()).orElseThrow(() -> new HttpClientErrorException(HttpStatus.NOT_FOUND, "Khong tim thay"));
    ...
    public ListResult<Specialities> filter(int size, int page, boolean desc, String orderBy) {
        ...
        return ListResult.from(specialitiesRepository.findAll(documentPartnerSpecification, pageable(page, size, orderBy, desc)));
    }
